...
if [ -d "Databases/$selected_database" ] && [ -r "Databases/$selected_database" ]; then
    ...
    if [ -f "Databases/$selected_database/$selected_table""_structure.txt" ]; then
        read -p "Do you want to retrieve all data from $selected_table? (yes/no): " retrieve_choice
        if [ "$retrieve_choice" = "yes" ]; then
            cat "Databases/$selected_database/$selected_table""_data.txt"
        else
            read -p "Enter the value you want to search for: " search_value
            grep "$search_value" "Databases/$selected_database/$selected_table""_data.txt"
        fi
    else
